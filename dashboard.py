...
app.layout = html.Div(
    className="container",
    style={'font-family': 'Inter, sans-serif', 'backgroundColor': '#f8fafc', 'color': '#0f172a', 'padding': '20px'},
    children=[
        ...
        html.Div(style={'display': 'flex', 'gap': '20px', 'margin-bottom': '24px'}, children=[
            html.Div(style={'flex': '1'}, children=[
                html.Label("Seleccionar Fecha", style={'font-weight': '600'}),
        ...
        # Gráfico 3: Histórico
        html.Div(children=[dcc.Graph(id="graph-historical")]),
...
def update_curves(fecha_sel, xaxis):
    ...
    fig_curve.update_layout(layout_light("Curva TIR vs Duración", xaxis.capitalize(), "TIR (%)"))
...
def update_hist(busqueda, tipo):
    ...
    fig.add_trace(go.Scatter(
        x=df_i['fecha'], y=df_i['spreads']*100 if 'spreads' in df_i.columns else np.zeros(len(df_i)),
        mode='lines+markers',
